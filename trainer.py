...
class Trainer(L.LightningModule):
    def __init__(self, config: DictConfig):
        ...
        if config.dep_former_name is not None:
            dep_former_weight_path = hf_hub_download(config.model_repo, config.dep_former_name)
    ...
    def training_step(self, batch, batch_idx):
        phone_seq, phone_emb_idx, audio_codes, semantic_labels, audio_labels, spk_templates = batch
        logits, audio_logits, rand_idx = self.model(phone_seq, phone_emb_idx, audio_codes, spk_templates)
    ...
    def configure_optimizers(self):
        ...
        scheduler = LinearWarmupDecayScheduler(
            optimizer=optimizer,
            warmup_steps=total_steps * self.config.warmup_epochs // self.config.max_epochs,
